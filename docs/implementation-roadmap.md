...
- 指数バックオフとジッターを実装する。
- 認証失敗時に自動送信を停止する。
- 2.4GHz Wi-Fiだけを設定対象とし、サンプルのプレースホルダー値を起動時に拒否する。
...
## 11. フェーズ9：Readyチェックと初期リリース確定

### 11.0 実装状況

`GET /health/ready`の実装をもって、初期リリースの実装範囲を完了とする。

### 11.1 目的

測定受付に必要な設定とPostgreSQLの利用可否を外部から確認できるようにし、初期リリースを確定する。

### 11.2 主な作業

- `GET /health/ready`を実装する。
- 測定受付の必須設定が揃っていることを確認する。
- PostgreSQLへ`SELECT 1`を実行し、成功時はHTTP 200、失敗時はHTTP 503を返す。
- Gemini Developer APIをReady判定へ含めない。
- Ready失敗時に秘密値や内部例外を応答・ログへ出さない。

### 11.3 完了条件

- 必須設定とPostgreSQLが利用可能な場合、`GET /health/ready`がHTTP 200を返す。
- 必須設定の不足またはPostgreSQL障害時、同EndpointがHTTP 503を返す。
- `GET /health/live`はDB障害時もFastAPIプロセスが応答可能ならHTTP 200を返す。
- Readyの単体テスト、pytest、Ruffおよびmypyが成功する。

暗号化バックアップ、隔離復元、CI、定量性能試験、包括的な障害・セキュリティ試験、
本番専用Secret機構および追加の運用自動化は、初期リリースの範囲に含めない。
