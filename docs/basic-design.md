...
| 文書版数 | 0.0.0 |
| 作成日 | 2026年8月11日 |
| 入力文書 | `docs/requirements.md` SoraSense 要件定義書 v0.0.0 |
| 下位文書 | `docs/detailed-design.md` SoraSense 詳細設計書 v0.0.0 |
| 文書目的 | 初期リリースの全体構造、主要方式、論理データ、画面、セキュリティ、運用およびテスト方針を定義する |

### 1.1 変更履歴

| 版数 | 日付 | 変更内容 |
|---|---|---|
| 0.0.0 | 2026年8月13日 | 版数管理開始時点の基本設計を登録 |
...
- 日時はUTCで保存・交換し、画面では利用者のタイムゾーンへ変換する。
...
利用者向けの閲覧APIは提供せず、GrafanaがPostgreSQLの参照用VIEWを直接参照する。Swagger UI、ReDocおよびOpenAPIエンドポイントは利用しない。具体的なパス、要求・応答、入力検証およびエラーは詳細設計書で定義する。
...
初期リリースでは次の3種類を永続化する。単一デバイスと単一利用者の認証情報は環境変数で管理する。

| データ | 主な情報 | 用途 |
|---|---|---|
| 測定データ | デバイスID、メッセージID、測定日時、温度、湿度、受信日時 | 履歴、集計、重複排除、状態判定 |
| アラート | デバイスID、項目、方向、状態、発生時の閾値・測定値、発生・解消日時 | 異常の継続管理と履歴表示 |
| AI利用履歴 | 質問、回答、処理結果、トークン利用量、エラー、実行日時 | 利用状況と障害の確認 |

3種類のデータ間に外部キー関係は設けない。物理テーブルのカラム、型、制約、索引は詳細設計書で定義する。

### 7.2 Grafana参照用データ

PostgreSQLの`reporting`スキーマに、最新値、デバイス状態、時系列、アラート履歴を提供するVIEWを配置する。GrafanaにはVIEWのSELECT権限だけを与え、基底テーブルの変更権限を与えない。
