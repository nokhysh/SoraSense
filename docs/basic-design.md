...
| PostgreSQL | 測定値、アラート、AI利用履歴の保存 | 永続volumeを維持して再起動する |
...
AI質問は一問一答とし、過去の質問・回答を次の質問へ渡さない。アプリケーションは外部AIを呼び出す前に、質問を最新値、統計、時系列、期間比較、アラート履歴、条件確認、未対応操作、対象外、ヘルプへ決定的に分類する。「昨日」「先週」等の相対期間と明示日付は、アプリケーションが注入可能な現在時刻と`Asia/Tokyo`を基準に半開区間へ解決し、Geminiが生成した期間ではなく解決済み期間を照会へ使用する。条件確認、未対応操作、対象外およびヘルプは固定回答を返し、Gemini Developer APIと照会サービスを利用しない。データ照会では分類に対応する参照専用ToolだけをAgentへ公開し、取得結果に基づいて回答する。回答には対象期間と主要な根拠値を含め、外部AIを利用した質問の利用量とエラーを記録する。Tool呼出しとAgent処理には上限を設ける。
...
Toolは生SQLや任意URLを受け取らず、構造化した結果を返す。Agentへ全Toolを一括公開せず、事前分類で回答に必要と判断した1種類だけを公開する。期間指定はアプリケーションの解決結果をデータアクセス境界で強制し、モデルが別期間を指定しても参照しない。具体的な分類条件、期間解決規則、Tool名、引数、件数制限は詳細設計書で定義する。
...
初期リリースでは業務データの自動削除を行わず継続保持する。手動削除の運用は初期リリースの対象外とする。
...
- Gemini Developer APIとの外部通信はHTTPSとし、APIキーはFree Tierプロジェクトに限定する。Geminiへの送信内容はデータ照会に分類した質問、Instructionsおよびその分類に対応するTool結果だけに制限する。固定回答で完結する質問はGeminiへ送信しない。
...
初期リリースではPostgreSQLの永続volumeを維持したサービス再起動を復旧範囲とする。日次バックアップ、暗号化、世代管理、隔離環境への復元およびRPO・RTO保証は将来設計とし、初期リリースへ含めない。

## 12. 性能・可用性設計

- 1台から1分間隔で送られる測定データを24時間継続処理できる構成とする。
- 定量的な性能保証は将来対象とし、初期リリースでは通常利用時の機能確認を行う。
...
| 障害 | DB停止時のReady判定、永続volumeを維持した再起動、外部AIのFake試験 |
| 性能 | 初期リリースでは通常利用時の機能確認 |
| セキュリティ | 認証、権限、入力検証、秘密情報露出 |

初期リリースでは静的解析、型チェック、単体・結合テストを開発環境で実行する。CIと依存脆弱性検査の自動化は将来対象とする。
...
| NFR-002～003 性能・独立性 | 4、7、12、13 | AC-001、AC-005 |
| NFR-010～011 可用性 | 3、11～13 | AC-005、AC-008 |
...
- 本番のリバースプロキシ、証明書、専用Secret機構、バックアップ保存先
