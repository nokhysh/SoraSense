...
|---|---|
| 文書版数 | 0.0.0 |
| 入力文書 | 要件定義書 v0.0.0、基本設計書 v0.1.0 |
...
`create_app(settings)`でFastAPIを生成する。Swagger UI、ReDoc、OpenAPIエンドポイントを無効化し、リクエストID、例外変換、構造化ログのMiddlewareを登録する。起動時に必須設定、閾値設定、DB接続を検証する。設定不正ではプロセスを起動失敗とし、値そのものをログへ出さない。
